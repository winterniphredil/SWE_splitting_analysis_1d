...
def exact_update_no_exp(u_0, h_0, k, Fr, c, n, dt):
    ...
    omega_plus = c*(Fr+1)
    omega_minus = c*(Fr-1)
    exact_exp = ( cos(1.*k*x) + k*omega_plus * sin(1.*k*x) * n * dt - 1/2 * (k*omega_plus)**2 * cos(1.*k*x) * n**2 * dt**2 - 1/6 * (k*omega_plus)**3 * sin(1.*k*x) * n**3 * dt**3 )
    ...
    return exact_exp*u_0, exact_exp*h_0
...
def pred_errors(sch, its, k, Fr, c, dt):
    ...
    with open("splitting_schemes_params_non_dim_exact_2.txt","r") as pred_file:
        ...
        for i in range(len(predicts)):
            ...
            new_pred = [(eval(j.split(" : ")[-1])*dt**(i+1)) for j in this_pred]
            predicts[i] = new_pred
